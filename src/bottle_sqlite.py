    ...
    def apply(self, callback, context):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # if response is streamed, close db handle after enumerating it
                # (as cursors may still be alive and necessary)
                # I don't know if there is a rock solid way to check for this
                if hasattr(rv, "__next__") or hasattr(rv, "next"):
                  def after():
                    db.close()
                    return
                    yield None
                  rv = itertools.chain(rv, after())
                else:
                  db.close()
            return rv
