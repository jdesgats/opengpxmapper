...
var Aristochart = function(element, options, theme) {
	...
	this.defaults = {
		...
		label: {
			...
			x: {
				step: 1,
        format: Aristochart.label.format,
			},
			y: {
				step: 1,
        format: Aristochart.label.format,
		...
		switch(feature) {
			...
			case "tick":
				if(defaults.tick.visible) {
					...
					for(var i = 0; i < Math.floor(stepX + 1); i++) that.options.tick.render.call(that, defaults, that.box.x  + (disX * i), (defaults.tick.x.fixed) ? axis.x.y1 : oy, "x", i);
					for(var i = 0; i < Math.floor(stepY + 1); i++) that.options.tick.render.call(that, defaults, (defaults.tick.y.fixed) ? axis.y.x1 : ox, that.box.y + (disY * i), "y", i);
				}
			break;
			...
			case "label":
					...
					if(defaults.label.x.visible)
						for(var i = 0; i < Math.floor(stepX + 1); i++) 
					...
					if(defaults.label.y.visible)
						for(var i = 0; i < Math.floor(stepY + 1); i++) {
							var pos = stepY - i,
...
Aristochart.label = {
	text: function(style, text, x, y, type, i) {
		if(i % this.options.label[type].step == 0) {
			...
			var text = this.options.label[type].format(text, i);
			this.ctx.fillText(text, x, y);
		}
	},
  format: function(number) {
    return (/(\-?\d+(\.\d)?)/.exec(number) || [ ])[0];
  }
};
